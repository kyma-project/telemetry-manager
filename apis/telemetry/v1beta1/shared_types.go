...
// +kubebuilder:validation:XValidation:rule="(has(self.authentication) && has(self.authentication.oauth2) && self.protocol == 'grpc') ? (has(self.tls) && self.tls.insecure != true) : true",message="OAuth2 authentication requires TLS to be configured when using gRPC protocol"
