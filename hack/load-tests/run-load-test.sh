...
function get_result_and_cleanup_trace() {
  ...
  QUERY_EXPORTED='query=round(sum(rate(otelcol_exporter_sent_spans_total{service="telemetry-trace-gateway-metrics"}[20m])))'
...
function get_result_and_cleanup_metric() {
    ...
    QUERY_EXPORTED='query=round(sum(rate(otelcol_exporter_sent_metric_points_total{service="telemetry-metric-gateway-metrics"}[20m])))'
