...
function create_module() {
    ...
    ${KYMA} alpha create module --module-config-file=module-config.yaml --registry ${MODULE_REGISTRY} -c oauth2accesstoken:${GCP_ACCESS_TOKEN} -o moduletemplate.yaml --module-archive-version-overwrite --ci
}
...
function create_dev_module() {
    ...
    ${KYMA} alpha create module --module-config-file=module-config-dev.yaml --registry ${MODULE_REGISTRY} -c oauth2accesstoken:${GCP_ACCESS_TOKEN} -o moduletemplate-dev.yaml --module-archive-version-overwrite --ci
}
