...
var _ = Describe(suite.ID(), Label(suite.LabelTelemetryLogsAnalysis), Ordered, func() {
	...
	Context("When OtelCollector-based components are deployed", func() {
		...
		It("Should have running backends", func() {
			assert.DeploymentShouldBeReady(ctx, k8sClient, types.NamespacedName{Namespace: otelCollectorNs, Name: otelCollectorLogBackendName})
			assert.DeploymentShouldBeReady(ctx, k8sClient, types.NamespacedName{Namespace: otelCollectorNs, Name: metricBackendName})
			assert.DeploymentShouldBeReady(ctx, k8sClient, types.NamespacedName{Namespace: otelCollectorNs, Name: traceBackendName})
		})

		It("Should have running pipelines", func() {
			assert.LogPipelineShouldBeHealthy(ctx, k8sClient, otelCollectorLogPipelineName)
			assert.MetricPipelineShouldBeHealthy(ctx, k8sClient, metricPipelineName)
			assert.TracePipelineShouldBeHealthy(ctx, k8sClient, tracePipelineName)
		})
		...
		It("Should push metrics successfully", func() {
			assert.MetricsFromNamespaceShouldBeDelivered(proxyClient, metricbackendExportURL, otelCollectorNs, telemetrygen.MetricNames)
		})

		It("Should push traces successfully", func() {
			assert.TracesFromNamespaceShouldBeDelivered(proxyClient, tracebackendExportURL, otelCollectorNs)
		})
	...
	Context("When FluentBit-based components are deployed", func() {
		...
		It("Should have a running backend", func() {
			assert.DeploymentShouldBeReady(ctx, k8sClient, types.NamespacedName{Namespace: fluentBitNs, Name: fluentBitLogBackendName})
		})

		It("Should have a running pipeline", func() {
			assert.LogPipelineShouldBeHealthy(ctx, k8sClient, fluentBitLogPipelineName)
		})
