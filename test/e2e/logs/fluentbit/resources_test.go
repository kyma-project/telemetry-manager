...
var _ = Describe(suite.ID(), Label(suite.LabelFluentBit), Ordered, func() {
	...
	Context("When a LogPipeline exists", Ordered, func() {
		...
		BeforeAll(func() {
			...
			DeferCleanup(func() {
				Expect(kitk8s.DeleteObjects(suite.Ctx, suite.K8sClient, &pipeline)).Should(Succeed())
			})
			Expect(kitk8s.CreateObjects(suite.Ctx, suite.K8sClient, &pipeline, secret.K8sObject())).Should(Succeed())
		})

		It("Should have a ServiceAccount owned by the LogPipeline", func() {
			var serviceAccount corev1.ServiceAccount
			assert.HasOwnerReference(suite.Ctx, suite.K8sClient, &serviceAccount, kitkyma.FluentBitServiceAccount, ownerReferenceKind, pipelineName)
		})

		It("Should have a ClusterRole owned by the LogPipeline", func() {
			var clusterRole rbacv1.ClusterRole
			assert.HasOwnerReference(suite.Ctx, suite.K8sClient, &clusterRole, kitkyma.FluentBitClusterRole, ownerReferenceKind, pipelineName)
		})

		It("Should have a ClusterRoleBinding owned by the LogPipeline", func() {
			var clusterRoleBinding rbacv1.ClusterRoleBinding
			assert.HasOwnerReference(suite.Ctx, suite.K8sClient, &clusterRoleBinding, kitkyma.FluentBitClusterRoleBinding, ownerReferenceKind, pipelineName)
		})

		It("Should have an exporter metrics Service owned by the LogPipeline", func() {
			var service corev1.Service
			assert.HasOwnerReference(suite.Ctx, suite.K8sClient, &service, kitkyma.FluentBitExporterMetricsService, ownerReferenceKind, pipelineName)
		})

		It("Should have a metrics Service owned by the LogPipeline", func() {
			var service corev1.Service
			assert.HasOwnerReference(suite.Ctx, suite.K8sClient, &service, kitkyma.FluentBitMetricsService, ownerReferenceKind, pipelineName)
		})

		It("Should have a telemetry-fluent-bit ConfigMap owned by the LogPipeline", func() {
			var configMap corev1.ConfigMap
			assert.HasOwnerReference(suite.Ctx, suite.K8sClient, &configMap, kitkyma.FluentBitConfigMap, ownerReferenceKind, pipelineName)
		})

		It("Should have a telemetry-fluent-bit-luascripts ConfigMap owned by the LogPipeline", func() {
			var configMap corev1.ConfigMap
			assert.HasOwnerReference(suite.Ctx, suite.K8sClient, &configMap, kitkyma.FluentBitLuaConfigMap, ownerReferenceKind, pipelineName)
		})

		It("Should have a telemetry-fluent-bit-parsers ConfigMap owned by the LogPipeline", func() {
			var configMap corev1.ConfigMap
			assert.HasOwnerReference(suite.Ctx, suite.K8sClient, &configMap, kitkyma.FluentBitParserConfigMap, ownerReferenceKind, pipelineName)
		})

		It("Should have a telemetry-fluent-bit-files ConfigMap owned by the LogPipeline", func() {
			var configMap corev1.ConfigMap
			assert.HasOwnerReference(suite.Ctx, suite.K8sClient, &configMap, kitkyma.FluentBitFilesConfigMap, ownerReferenceKind, pipelineName)
		})

		It("Should have a DaemonSet owned by the LogPipeline", func() {
			var daemonSet appsv1.DaemonSet
			assert.HasOwnerReference(suite.Ctx, suite.K8sClient, &daemonSet, kitkyma.FluentBitDaemonSetName, ownerReferenceKind, pipelineName)
		})

		It("Should have a Network Policy owned by the LogPipeline", func() {
			var networkPolicy networkingv1.NetworkPolicy
			assert.HasOwnerReference(suite.Ctx, suite.K8sClient, &networkPolicy, kitkyma.FluentBitNetworkPolicy, ownerReferenceKind, pipelineName)
		})
