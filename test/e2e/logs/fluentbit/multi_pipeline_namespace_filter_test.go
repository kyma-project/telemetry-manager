...
var _ = Describe(suite.ID(), Label(suite.LabelLogsFluentBit), Ordered, func() {
	Context("When multiple log pipelines with namespace filter exist", Ordered, func() {
		...
		makeResources := func() []client.Object {
			...
			backend1 := backend.New(mock1Ns, backend.SignalTypeLogsFluentBit, backend.WithName(backendIncludeNamespaceName))

			logProducer1 := loggen.New(mock1Ns)
			backendIncludeNamespaceExportURL = backend1.ExportURL(suite.ProxyClient)
			...
			backend2 := backend.New(mock2Ns, backend.SignalTypeLogsFluentBit, backend.WithName(backendExcludeNamespaceName))
		...
		It("Should have a running logpipelines", func() {
			assert.LogPipelineHealthy(suite.Ctx, suite.K8sClient, pipelineIncludeNamespaceName)
			assert.LogPipelineHealthy(suite.Ctx, suite.K8sClient, pipelineExcludeNamespaceName)
		})
		...
		It("Should have a log backend running", func() {
			assert.DeploymentReady(suite.Ctx, suite.K8sClient, types.NamespacedName{Namespace: mock1Ns, Name: backendIncludeNamespaceName})
			assert.DeploymentReady(suite.Ctx, suite.K8sClient, types.NamespacedName{Namespace: mock2Ns, Name: backendExcludeNamespaceName})
		})

		It("Log pipeline include namespace should have logs from expected namespaces", func() {
			Eventually(func(g Gomega) {
				resp, err := suite.ProxyClient.Get(backendIncludeNamespaceExportURL)
