...
var _ = Describe("Logging", Label("logging"), func() {
	...
	Context("Log delivery to custom output", Ordered, func() {
		var (
			urls               *urlprovider.URLProvider
			mockDeploymentName = "log-receiver"
			mockNs             = "custom-output-log-delivery"
		)

		BeforeAll(func() {
			k8sObjects, logsURLProvider := makeCustomOutputLogTestK8sObjects(mockNs, mockDeploymentName)
		...
		It("Should verify end-to-end log delivery", func() {
			Eventually(func(g Gomega) {
...
func makeCustomOutputLogTestK8sObjects(namespace string, mockDeploymentName string) ([]client.Object, *urlprovider.URLProvider) {
	var (
		objs []client.Object
