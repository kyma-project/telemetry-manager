...
var _ = Describe(suite.ID(), Label(suite.LabelLogs), Ordered, func() {
	...
	Context("When a log pipeline with TLS Cert expiring within 2 weeks is activated", Ordered, func() {
		...
		It("Should have a tlsCertAboutToExpire Condition set in pipeline conditions", func() {
			assert.LogPipelineHasCondition(ctx, k8sClient, pipelineName, conditions.TypeConfigurationGenerated, conditions.ReasonTLSCertificateAboutToExpire)
		})

		It("Should have telemetryCR showing correct condition in its status", func() {
			assert.TelemetryHasWarningState(ctx, k8sClient)
			assert.TelemetryHasCondition(ctx, k8sClient, "LogComponentsHealthy", conditions.ReasonTLSCertificateAboutToExpire, true)
		})
