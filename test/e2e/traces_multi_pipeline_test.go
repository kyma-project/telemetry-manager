...
var _ = Describe("Traces Multi-Pipeline", Label("traces"), func() {
	Context("When multiple tracepipelines exist", Ordered, func() {
		...
		makeResources := func() []client.Object {
			...
			for _, backendName := range []string{mockBackendName1, mockBackendName2} {
				...
				pipeline := kitk8s.NewTracePipeline(fmt.Sprintf("%s-%s", mockBackend.Name(), "pipeline")).WithOutputEndpointFromSecret(mockBackend.HostSecretRefv1alpha1())
	...
	Context("When a broken tracepipeline exists", Ordered, func() {
		...
		makeResources := func() []client.Object {
			...
			healthyPipeline := kitk8s.NewTracePipeline("healthy").WithOutputEndpointFromSecret(mockBackend.HostSecretRefv1alpha1())
			...
			brokenPipeline := kitk8s.NewTracePipeline("broken").WithOutputEndpointFromSecret(unreachableHostSecret.SecretKeyRefV1Alpha1("metric-host"))
