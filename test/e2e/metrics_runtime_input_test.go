...
var (
	...
	kubeletMetricResourceAttributes = []string{"k8s.cluster.name", "k8s.container.name", "k8s.daemonset.name", "k8s.deployment.name", "k8s.namespace.name", "k8s.node.name", "k8s.pod.name", "k8s.pod.uid", "kyma.source"}
	...
	Context("When a metricpipeline exists", Ordered, func() {
		...
		It("Should verify kubelet metric names", func() {
			Eventually(func(g Gomega) {
				...
				g.Expect(resp).To(HaveHTTPBody(SatisfyAll(
					HaveMetricsWithNames(kubeletMetricNames...))))
			}, timeout, interval).Should(Succeed())
		})
		...
		It("Should verify kubelet metric attributes", func() {
			Eventually(func(g Gomega) {
				...
				g.Expect(resp).To(HaveHTTPStatus(http.StatusOK))
				g.Expect(resp).To(HaveHTTPBody(SatisfyAll(
					HaveMetricsWithResourceAttributes(kubeletMetricResourceAttributes...))))
			}, timeout, interval).Should(Succeed())
		})
	})
})
