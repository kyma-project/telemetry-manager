	...
	Context("When a LogPipeline exists", Ordered, func() {
		...
		It("Should clean up log pipeline resources when pipeline becomes non-reconcilable", func() {
			...
			Eventually(func(g Gomega) bool {
				...
				return apierrors.IsNotFound(err)
			}, periodic.EventuallyTimeout, periodic.DefaultInterval).Should(BeTrue(), "Exporter metrics service should be deleted")
			...
			Eventually(func(g Gomega) bool {
				...
				return apierrors.IsNotFound(err)
			}, periodic.EventuallyTimeout, periodic.DefaultInterval).Should(BeTrue(), "Network policy should be deleted")

			Eventually(func(g Gomega) bool {
				var configMap corev1.ConfigMap
				err := k8sClient.Get(ctx, kitkyma.FluentBitConfigMap, &configMap)
				return apierrors.IsNotFound(err)
			}, periodic.EventuallyTimeout, periodic.DefaultInterval).Should(BeTrue(), "ConfigMap should be deleted")

			Eventually(func(g Gomega) bool {
				var configMap corev1.ConfigMap
				err := k8sClient.Get(ctx, kitkyma.FluentBitLuaConfigMap, &configMap)
				return apierrors.IsNotFound(err)
			}, periodic.EventuallyTimeout, periodic.DefaultInterval).Should(BeTrue(), "Lua ConfigMap should be deleted")

			Eventually(func(g Gomega) bool {
				var configMap corev1.ConfigMap
				err := k8sClient.Get(ctx, kitkyma.FluentBitParserConfigMap, &configMap)
				return apierrors.IsNotFound(err)
			}, periodic.EventuallyTimeout, periodic.DefaultInterval).Should(BeTrue(), "Parser ConfigMap should be deleted")

			Eventually(func(g Gomega) bool {
				var configMap corev1.ConfigMap
				err := k8sClient.Get(ctx, kitkyma.FluentBitSectionsConfigMap, &configMap)
				return apierrors.IsNotFound(err)
			}, periodic.EventuallyTimeout, periodic.DefaultInterval).Should(BeTrue(), "Sections ConfigMap should be deleted")

			Eventually(func(g Gomega) bool {
				var configMap corev1.ConfigMap
				err := k8sClient.Get(ctx, kitkyma.FluentBitFilesConfigMap, &configMap)
				return apierrors.IsNotFound(err)
			}, periodic.EventuallyTimeout, periodic.DefaultInterval).Should(BeTrue(), "Files ConfigMap should be deleted")

			Eventually(func(g Gomega) bool {
				var daemonSet appsv1.DaemonSet
				err := k8sClient.Get(ctx, kitkyma.FluentBitDaemonSet, &daemonSet)
				return apierrors.IsNotFound(err)
			}, periodic.EventuallyTimeout, periodic.DefaultInterval).Should(BeTrue(), "DaemonSet should be deleted")
