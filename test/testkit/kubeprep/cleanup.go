...
func CleanupCluster(ctx context.Context, k8sClient client.Client, cfg Config) error {
	log.Println("Cleaning up cluster resources...")

	// 1. Undeploy telemetry manager (unless skipped)
	if !cfg.SkipManagerDeployment {
		if err := undeployManager(ctx, k8sClient, cfg); err != nil {
			log.Printf("Warning: failed to undeploy telemetry manager: %v", err)
		}
	}

	// 2. Clean up test prerequisites
	if !cfg.SkipPrerequisites {
		if err := cleanupPrerequisites(ctx, k8sClient); err != nil {
			log.Printf("Warning: failed to cleanup prerequisites: %v", err)
		}
	}

	// 3. Delete all pipeline CRs
	if err := deletePipelines(ctx, k8sClient); err != nil {
		log.Printf("Warning: failed to delete pipelines: %v", err)
	}

	// 4. Delete test namespaces
	if err := deleteTestNamespaces(ctx, k8sClient); err != nil {
		log.Printf("Warning: failed to delete test namespaces: %v", err)
	}
	...
	if isIstioInstalled(ctx, k8sClient) {
		log.Println("Istio detected in cluster, removing...")
		if err := uninstallIstio(ctx, k8sClient); err != nil {
			log.Printf("Warning: failed to uninstall Istio: %v", err)
		}
	} else {
		log.Println("Istio not detected, skipping Istio cleanup")
	}

	log.Println("Cluster cleanup complete")
	return nil
}
...
func deletePipelines(ctx context.Context, k8sClient client.Client) error {
	log.Println("Deleting pipeline resources...")
	...
	for _, gvr := range pipelineTypes {
		if err := deleteAllResources(ctx, k8sClient, gvr); err != nil {
			log.Printf("Warning: failed to delete %s: %v", gvr.Resource, err)
		}
	}
...
func deleteAllResources(ctx context.Context, k8sClient client.Client, gvr schema.GroupVersionResource) error {
	...
	for _, item := range list.Items {
		if err := k8sClient.Delete(ctx, &item); err != nil && !apierrors.IsNotFound(err) {
			log.Printf("Warning: failed to delete %s %s: %v", gvr.Resource, item.GetName(), err)
		}
	}
...
func deleteTestNamespaces(ctx context.Context, k8sClient client.Client) error {
	log.Println("Deleting test namespaces...")
	...
	for _, ns := range namespaces.Items {
		...
		if strings.HasPrefix(ns.Name, "test-") ||
			...
			if err := k8sClient.Delete(ctx, &ns, &client.DeleteOptions{
				...
				log.Printf("Warning: failed to delete namespace %s: %v", ns.Name, err)
			}
		}
	}
...
func ForceDeleteNamespace(ctx context.Context, k8sClient client.Client, name string) error {
	ns := &corev1.Namespace{}
	ns.Name = name
	...
	if err := k8sClient.Get(ctx, client.ObjectKey{Name: name}, ns); err == nil {
		if ns.Status.Phase == corev1.NamespaceTerminating && len(ns.Finalizers) > 0 {
			log.Printf("Removing finalizers from namespace %s", name)
