...
func TestBuildConfigShuffled(t *testing.T) {
	...
	config1, _, err := sut.Build(t.Context(), []telemetryv1beta1.TracePipeline{pipelines[0], pipelines[1]}, buildOptions)
	require.NoError(t, err)

	config2, _, err := sut.Build(t.Context(), []telemetryv1beta1.TracePipeline{pipelines[1], pipelines[0]}, buildOptions)
	require.NoError(t, err)
