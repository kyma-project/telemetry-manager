...
func TestMakeRules(t *testing.T) {
	...
	require.Equal(t, "sum by (exporter) (rate(otelcol_exporter_sent_spans{service=\"telemetry-trace-gateway-metrics\"}[5m])) > 0", ruleGroup.Rules[5].Expr)

	require.Equal(t, "TraceGatewayExporterDroppedData", ruleGroup.Rules[6].Alert)
	require.Equal(t, "sum by (exporter) (rate(otelcol_exporter_send_failed_spans{service=\"telemetry-trace-gateway-metrics\"}[5m])) > 0", ruleGroup.Rules[6].Expr)
	...
	require.Equal(t, "sum by (exporter) (rate(otelcol_exporter_enqueue_failed_spans{service=\"telemetry-trace-gateway-metrics\"}[5m])) > 0", ruleGroup.Rules[8].Expr)
