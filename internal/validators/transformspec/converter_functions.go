// Mainly copied over from "github.com/open-telemetry/opentelemetry-collector-contrib/internal/filter/filterottl/functions.go"
...
func spanEventConverterFuncs() map[string]ottl.Factory[ottlspanevent.TransformContext] {
	return ottlfuncs.StandardConverters[ottlspanevent.TransformContext]()
}

func metricConverterFuncs() map[string]ottl.Factory[ottlmetric.TransformContext] {
...
func dataPointConverterFuncs() map[string]ottl.Factory[ottldatapoint.TransformContext] {
	return ottlfuncs.StandardConverters[ottldatapoint.TransformContext]()
}

func scopeConverterFuncs() map[string]ottl.Factory[ottlscope.TransformContext] {
	return ottlfuncs.StandardConverters[ottlscope.TransformContext]()
}

func logConverterFuncs() map[string]ottl.Factory[ottllog.TransformContext] {
	return ottlfuncs.StandardConverters[ottllog.TransformContext]()
}

func resourceConverterFuncs() map[string]ottl.Factory[ottlresource.TransformContext] {
...
func checkDataPoints(tCtx ottlmetric.TransformContext, key string, expectedVal *string) (any, error) {
	...
	//exhaustive:enforce
	//nolint:exhaustive // copied over from upstream
	switch metric.Type() {
