...
func Test_EvaluateTLSCertCondition(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expectedStatus:  metav1.ConditionFalse,
			expectedReason:  ReasonTLSCertificateExpired,
			expectedMessage: fmt.Sprintf("TLS CA expired on %s", "2000-02-01"),
		},
		{
			...
			expectedStatus:  metav1.ConditionTrue,
			expectedReason:  ReasonTLSCertificateAboutToExpire,
			expectedMessage: fmt.Sprintf("TLS CA is about to expire, configured CA is valid until %s", time.Now().AddDate(0, 0, 7).Format(time.DateOnly)),
		},
		{
