...
func TestNoDuplicateFileNameInSamePipeline(t *testing.T) {

	pipelineValidator := &Validator{}

	newPipeline := testutils.NewLogPipelineBuilder().WithName("foo").WithFile("f1.json", "").WithFile("f2.json", "").Build()
	err := pipelineValidator.Validate(t.Context(), &newPipeline)
...
func TestDuplicateFileNameInSamePipeline(t *testing.T) {
	...
	require.EqualError(t, err, "duplicate file names detected please review your pipeline")
}
