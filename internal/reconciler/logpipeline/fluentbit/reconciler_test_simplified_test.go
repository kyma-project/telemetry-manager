...
func TestFIPSModeTransition(t *testing.T) {
	t.Run("FIPS mode transition from disabled to enabled cleans up resources", func(t *testing.T) {
...
func TestComplexFIPSTransitionWithMockVerification(t *testing.T) {
	t.Run("FIPS transition with resource management verification", func(t *testing.T) {
		...
		case <-deleteCalledChan:
			t.Log("✓ DeleteResources was called as expected during FIPS enabled phase")
		case <-time.After(100 * time.Millisecond):
		...
		t.Log("✓ FIPS mode transition test completed successfully")
		t.Log("✓ Verified that resources are properly cleaned up during FIPS mode transition")
	})
}
