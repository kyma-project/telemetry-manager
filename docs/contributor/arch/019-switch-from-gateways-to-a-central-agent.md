...
A zero-downtime rollout of a `DaemonSet` can be achieved by using the `RollingUpdate` update strategy with `maxUnavailable: 0` and `maxSurge: 1`, as shown below:
...
This setup was tested using the OpenTelemetry Demo application to verify that no telemetry data is lost during the rollout. Tests were executed for all three signal types—metrics, traces, and logs. The demo application, which includes services written in various programming languages and using the OTel SDK, continued sending telemetry to the test collector throughout the rollout without any data loss.
