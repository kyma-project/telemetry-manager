...
| Prometheus Scrape Config | `app-services` job scrapes services with `prometheus.io/scrape: "true"` annotation | The Metric Agent must scrape application metrics from workloads that expose plain-text Prometheus endpoints. When Istio is detected and Prometheus input is enabled, this job drops HTTPS targets (identified by `security.istio.io/tlsMode: istio` pod label or `prometheus.io/scheme: https` service annotation) because those require mTLS certificates and are handled by the `app-services-secure` job instead. When Istio is not detected or Prometheus input is disabled, this job scrapes all targets. |
...
| OTel Processor           | `istio_noise_filter` (metric pipelines)                                                               | Filters out noisy Istio-related metrics (verbose Envoy metrics, internal mesh control plane data) to reduce data volume and improve signal quality for metric pipelines.                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `IngestOnly` | `sidecar.istio.io/inject: "false"` | (none)                                            | DestinationRule (TLS DISABLE)                        |
| `ExportOnly` | `sidecar.istio.io/inject: "true"`  | `traffic.sidecar.istio.io/includeInboundPorts: ""` | NetworkPolicy (15090)                                |
