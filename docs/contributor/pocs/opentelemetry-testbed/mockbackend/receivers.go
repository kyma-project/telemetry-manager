...
func (bor *BaseOTLPDataReceiver) Start(tc consumer.Traces, mc consumer.Metrics, lc consumer.Logs) error {
	...
	if bor.exporterType == "otlp" {
		...
		cfg.GRPC.Get().NetAddr = confignet.AddrConfig{Endpoint: fmt.Sprintf("0.0.0.0:%d", bor.Port), Transport: confignet.TransportTypeTCP}
	} else {
		if err := InsertDefault(&cfg.HTTP); err != nil {
			return err
		}
		cfg.HTTP.Get().ServerConfig.Endpoint = fmt.Sprintf("0.0.0.0:%d", bor.Port)
	}
...
func (bor *BaseOTLPDataReceiver) GenConfigYAMLStr() string {
	addr := fmt.Sprintf("0.0.0.0:%d", bor.Port)
