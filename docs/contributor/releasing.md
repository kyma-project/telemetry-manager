...
5. Bump the `telemetry-manager/{RELEASE_BRANCH}` branch with the new versions for the dependent images.
   ...
   - Update Docker images in the `.env` file:
      ...
      - Update the `ENV_FLUENTBIT_EXPORTER_IMAGE` variable with the corresponding tag for the main image from [europe-docker.pkg.dev/kyma-project/prod/directory-size-exporter:main](europe-docker.pkg.dev/kyma-project/prod/directory-size-exporter:main)
      - Update the `ENV_SELFMONITOR_IMAGE` variable with the corresponding tag for the main image from [europe-docker.pkg.dev/kyma-project/prod/telemetry-self-monitor:main](europe-docker.pkg.dev/kyma-project/prod/telemetry-self-monitor:main)
   - `make generate`
