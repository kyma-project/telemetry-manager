...
function setup() {
    ...
    # Deploy test setup
    kubectl apply -f trace-backpressure-test-setup.yaml
}
...
function cleanup() {
    ...
    RESULTS_TRACE_FILENAME="${RESULTS_DIRECTORY}/otel-${OTEL_VERSION}_traces_backpressure_test.csv"
    ...
    kubectl delete -f trace-backpressure-test-setup.yaml

    helm delete -n ${PROMETHEUS_NAMESPACE} ${HELM_PROM_RELEASE}
}

echo "Single TracePipeline Backpressure Test"
echo "--------------------------------------------"
trap cleanup EXIT
setup
wait_for_resources
