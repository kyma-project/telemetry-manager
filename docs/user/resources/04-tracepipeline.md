...
| ConfigurationGenerated | False            | OTTLSpecInvalid                 | OTTL specification is invalid, <FilterSpec/TransformSpec>: `reason`. Fix the syntax error indicated by the message or see troubleshooting: [Generic EOF Error Message When Using OTTL Transform or Filter](https://kyma-project.io/#/telemetry-manager/user/02-logs?id=generic_eof_error_message_when_using_ottl_transform_or_filter) |
