...
- **Throughput**:
  ...
  - For example, to support scenarios like a `Large` instance of SAP Cloud Logging (up to 30,000 LPS), scaling out the gateway by increasing the number of replicas to 4 instances, raises the throughput to about 20,600 LPS.
  - For tailing logs from stdout using the `runtime` input, the log agent is used instead, which runs one instance per node. When writing logs of an average size of 2KB to stdout, a single log agent instance can process approximately 1,000 LPS.
