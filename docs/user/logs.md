...
- **Throughput**:
  ...
  - For tailing logs from stdout using the `runtime` input, the log agent is used instead, which runs one instance per node. When writing logs of an average size of XKB to stdout, a single log agent instance can process approximately Y LPS.
