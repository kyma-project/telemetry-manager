...
You must specify the full path for every field, starting from the top-level context (resource, span, log, and so on). Short-hand references are not supported.
