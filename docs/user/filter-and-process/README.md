...
- **Source-level filtering** (Istio `Telemetry` CRD): For Istio-generated data (access logs and traces), configure the Istio `Telemetry` resource itself to control which workloads generate data and at what volume (sampling rate). For details, see [Filter Logs](filter-logs.md) and [Filter Traces](filter-traces.md).
