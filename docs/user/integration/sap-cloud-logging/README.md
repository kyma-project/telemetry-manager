...
For visualization of the metrics generated by the inputs of a MetricPipeline, the following Dashboards are available for a manual upload. In the SAP Cloud Logging Dashboard navigate to **Stack Managament > Saved Objects** and click the **Import** button to upload the files listed below:
...
| Istio Metrics | [Kyma Istio Service Metrics](https://raw.githubusercontent.com/kyma-project/telemetry-manager/main/docs/user/integration/sap-cloud-logging/dashboard-istio.ndjson) | Visualizes the Istio metrics of pods having an active Istio sidecar injection. The metrics are collected by the MetricPipeline `istio` input. |
