...
To ingest the application logs from within your cluster to Loki, use Kyma's [LogPipeline](./../../logs/README.md) feature based on OTLP. It enables the `application` input to automatically tail application logs from `stdout/stderr`. If you want to push logs natively, additionally use the [log gateway endpoint](./../../pipelines/otlp-input.md).
