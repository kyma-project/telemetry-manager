...
3. Kubernetes resources are generated using `ApplierDeleters` in `internal/resources/` — avoid inline resource construction in reconcilers
...
- Check that new CRD fields have corresponding DeepCopy generation (using `make generate`)
...
- Flag direct use of `client.Update` where `client.Patch` (status subresource) must be used for status updates
...
- New or changed environment variables must be documented in `docs/` or the relevant reference page — flag any that are only declared in code without a corresponding docs update
