...
.PHONY: e2e-test-logging-dev
e2e-test-logging-dev: ginkgo k3d test-matchers-logging ## Provision k3d cluster, deploy development variant and run end-to-end logging tests.
	K8S_VERSION=$(ENVTEST_K8S_VERSION) hack/provision-test-env.sh
...
.PHONY: e2e-test-tracing-dev
e2e-test-tracing-dev: ginkgo k3d test-matchers-tracing ## Provision k3d cluster, deploy development variant and run end-to-end tracing tests.
	K8S_VERSION=$(ENVTEST_K8S_VERSION) hack/provision-test-env.sh
	IMG=k3d-kyma-registry:5000/telemetry-manager:latest make deploy-dev
...
.PHONY: e2e-test-metrics-dev
 e2e-test-metrics-dev: ginkgo k3d test-matchers-metrics ## Provision k3d cluster, deploy development variant and run end-to-end metrics tests.
	K8S_VERSION=$(ENVTEST_K8S_VERSION) hack/provision-test-env.sh
	IMG=k3d-kyma-registry:5000/telemetry-manager:latest make deploy-dev
...
e2e-test-logging-release: ginkgo k3d test-matchers-logging ## Provision k3d cluster, deploy release (default) variant and run end-to-end logging tests.
	K8S_VERSION=$(ENVTEST_K8S_VERSION) hack/provision-test-env.sh
	IMG=k3d-kyma-registry:5000/telemetry-manager:latest make deploy-release
	$(GINKGO) run --tags e2e -v --junit-report=junit.xml --label-filter="logging" ./test/e2e
	mkdir -p ${ARTIFACTS}
	mv junit.xml ${ARTIFACTS}
...
deploy-release: manifests kustomize ## Deploy resources based on the release (default) variant to the K8s cluster specified in ~/.kube/config.
	cd config/manager && $(KUSTOMIZE) edit set image controller=${IMG}
	$(KUSTOMIZE) build config/default | kubectl apply -f -

.PHONY: undeploy-release
undeploy-release: ## Undeploy resources based on the release (default) variant from the K8s cluster specified in ~/.kube/config. Call with ignore-not-found=true to ignore resource not found errors during deletion.
